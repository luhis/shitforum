...
namespace UnitTests.Pages
{
    public class ThreadShould
    {
        ...
        public void AllowGet()
        {
            ...
            this.threadService.Setup(a => a.GetThread(threadId, 100, this.ct)).ReturnsT(
            ...
            this.thread.OnGet("bee", threadId, Guid.NewGuid(), CancellationToken.None).Wait();
        ...
        public void AllowPostValid()
        {
            ...
            this.thread.Post = new AddPost(threadId, "Matt", "sage", "comment", null);
            ...
            this.postService.Setup(a => a.Add(It.IsAny<Guid>(), threadId, It.IsAny<TripCodedName>(), "comment", true, It.IsAny<IIpHash>(), Option.None<Domain.File>(), this.ct)).Returns(
                Task.FromResult<OneOf<Success, Banned, ImageCountExceeded, PostCountExceeded>>(new Success()));
            this.threadService.Setup(a => a.GetThread(threadId, 100, this.ct)).ReturnsT(Option.Some(new ThreadDetailView(threadId, "aaa", new ThreadStats(10, 1, 10, 1), new BoardOverView(Guid.NewGuid(), "bbbb", "b"), new List<PostOverView>() )));
            this.bannedImageLogger.Setup(a => a.Log(null, IPAddress.Loopback, It.IsAny<IIpHash>()));

            this.thread.OnPostAsync(this.ct).Wait();
        ...
        public void AllowPostValidWithFile()
        {
            ...
            this.thread.Post = new AddPost(threadId, "Matt", "sage", "comment", file.Object);
            this.getIp.Setup(a => a.GetIp(It.IsAny<HttpRequest>())).Returns(IPAddress.Loopback);
            this.cookieStorage.Setup(a => a.SetNameCookie(It.IsAny<HttpResponse>(), "Matt"));
            this.threadService.Setup(a => a.GetThread(threadId, 100, this.ct)).ReturnsT(Option.Some(new ThreadDetailView(threadId, "aaa", new ThreadStats(10, 1, 10, 1), new BoardOverView(Guid.NewGuid(), "bbbb", "b"), new List<PostOverView>())));
            this.bannedImageLogger.Setup(a => a.Log(null, IPAddress.Loopback, It.IsAny<IIpHash>()));
            this.postService.Setup(a => a.Add(It.IsAny<Guid>(), threadId, It.IsAny<TripCodedName>(), "comment", true, It.IsAny<IIpHash>(), It.IsAny<Option<Domain.File>>(), this.ct))
        ...
        public void AllowPostInvalid()
        {
            ...
            this.thread.Post = new AddPost(threadId, "", "", "", null);
            this.thread.ModelState.AddModelError("file", "blah");

            this.iIsAdmin.Setup(a => a.IsAdmin(It.IsAny<HttpContext>())).Returns(false);
            this.getIp.Setup(a => a.GetIp(It.IsAny<HttpRequest>())).Returns(IPAddress.Loopback);
            this.threadService.Setup(a => a.GetThread(threadId, 100, this.ct)).Returns(
